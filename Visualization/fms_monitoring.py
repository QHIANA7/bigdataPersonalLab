...
def update_fms_metrics(spark):
    ...
    print(f"🔄 최신 'fms' 데이터를 찾습니다... (경로: {partition_path})")
    ...
    try:
        ...
        print(f"✅ {df_latest.count()}개 'fms' 장비의 최신 메트릭을 성공적으로 업데이트했습니다.")

    except Exception as e:
        if "Path does not exist" in str(e):
            print(f"⚠️ HDFS 경로를 찾을 수 없습니다: {partition_path}. 'fms' 데이터가 아직 없을 수 있습니다.")
        else:
...
def main():
    ...
    print(f"🚀 'fms' 데이터 프로메테우스 메트릭 서버를 시작합니다. (포트: {PROMETHEUS_PORT})")
